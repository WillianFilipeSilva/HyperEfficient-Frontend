window.addEventListener("DOMContentLoaded", () => {
  ...
  async function carregarInformacoesUsuario() {
    ...
    try {
      const usuario = await window.API_CONFIG.authenticatedRequest(
  ...
  async function salvarAlteracoes() {
    ...
    try {
      const response = await API_CONFIG.authenticatedRequest(
        `/usuarios/${usuarioOriginal.id}`,
        {
          method: "PUT",
      ...
      const usuarioAtualizado = await API_CONFIG.authenticatedRequest(
        `/usuarios/${usuarioOriginal.id}`
      );
